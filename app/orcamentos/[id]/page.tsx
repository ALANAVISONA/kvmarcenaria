...
// yyyy-mm-dd -> dd/mm/yyyy (para o PDF)
...
export default function QuoteDetailPage() {
  ...
  // Objeto do PDF (sempre atualizado)
  const orcamentoPDF = useMemo(() => {
    return {
      numero: String(quote?.order_number ?? quoteId),
      cliente: nomeCliente,
      status: status || "Orçamento",
      data: formatDateBR(quoteDate) || formatDateBR(quote?.quote_date ?? "") || "",
      itens: items.map((it) => {
        const nome = it.products?.name ?? "Produto";
      ...
      <Topbar
        ...
        right={
          <>
            ...
            {/* BOTÃO PDF */}
            <BotaoBaixarPDF orcamento={orcamentoPDF} />
                      ...
                      <div style={{ minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 12.5, color: UI.muted, marginTop: 6 }}>
                          ...
                          {it.products?.category ? (
                            <>
                              {" "}
                              • <span style={{ opacity: 0.8 }}>{it.products.category}</span>
                            </>
                          ) : null}
                        </div>
                      </div>
